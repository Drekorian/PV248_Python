...
class TestParseKarma(unittest.TestCase):
	...
	def test_plus(self):
		self.bot.parse("foo++")
		self.bot.parse("foo++")
		self.assertEqual(karma.KARMA.get("foo"), 2)

	def test_minus(self):
		karma.KARMA["foo"] = 2
		self.bot.parse("foo--")
		self.assertEqual(karma.KARMA.get("foo"), 1)

	def test_minus_zero(self):
		karma.KARMA["foo"] = 1
		self.bot.parse("foo--")
		self.assertEqual(karma.KARMA.get("foo"), None)

	def test_cpp(self):
		self.bot.parse("c++++")
		self.assertEqual(karma.KARMA.get("c++"), 1)
		self.bot.parse("c++--")
		self.assertEqual(karma.KARMA.get("c++"), None)

	def test_middle_line_plus(self):
		self.bot.parse("a b c d++ e")
		self.assertEqual(karma.KARMA.get("a b c d"), None)
		self.assertEqual(karma.KARMA.get("d"), 1)

	def test_middle_line_minus(self):
		karma.KARMA["d"] = 1
		self.bot.parse("a b c d--")
		self.assertEqual(karma.KARMA.get("a b c d"), None)
		self.assertEqual(karma.KARMA.get("d"), None)
...
class TestAPI(unittest.TestCase):
	...
	def test_interface_import(self):
		found = 0
		if "interface" in ircbot.__dict__:
			found += 1
		if "IRCBotShellInterface" in ircbot.__dict__:
			found += 1

		self.assertEquals(found, 1, msg="Zkontrolujte si pritomnost import interface nebo from interface import IRCBotShellInterface")

	def test_plugins_import(self):
		found = 0
		if "plugins" in ircbot.__dict__:
			found += 1
		if ("FILTERS" in ircbot.__dict__) and ("COMMANDS" in ircbot.__dict__):
			found += 1

		self.assertEquals(found, 1, msg="Zkontrolujte si pritomnost importu z plugins")


	def test_bot_using_plugins_for_commands(self):
		self.assertEquals(plugins.COMMANDS, ircbot.IrcBot.COMMANDS, msg="Zkontrolujte si jestli vas IrcBot pouziva stale self.COMMANDS a ma je nainicializovane z plugins.COMMANDS")

	def test_bot_using_plugins_for_filters(self):
		self.assertEquals(plugins.FILTERS, ircbot.IrcBot.FILTERS, msg="Zkontrolujte si jestli vas IrcBot pouziva stale self.FILTERS a ma je nainicializovane z plugins.FILTERS")
